...
export function GoalsPageClient({ goals, teamMembers=[] }: { goals: GoalRow[]; teamMembers?: TeamMember[] }) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#f8f9fa] overflow-hidden">
      {/* Header */}
      <div className="px-8 pt-6 pb-4 border-b border-slate-200 bg-white shrink-0 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-1">
            {["all","on_track","at_risk","off_track","achieved"].map(f=>(
              <button key={f} onClick={()=>setStatusFilter(f)} className={`px-3 py-1.5 text-[13px] font-medium rounded-md transition-colors ${statusFilter===f?"bg-slate-900 text-white":"text-slate-500 hover:bg-slate-100"}`}>
        ...
        <div className="flex items-center gap-3">
          {isFilterActive ? (
            <button onClick={() => { setSearch(''); setStatusFilter('all'); setColFilters({}); }} className="text-[11px] text-indigo-600 font-medium hover:text-indigo-800 transition-colors flex items-center gap-1 mr-2 bg-indigo-50 px-2 py-1 rounded">
              <X className="w-3 h-3" /> Clear filters
            </button>
          ) : null}
          <div className="relative w-72">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400"/>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search goals or owners..." className="w-full pl-8 pr-4 py-2 text-[12px] bg-white border border-slate-200 rounded-md outline-none focus:border-slate-400 shadow-sm"/>
          </div>
          <button onClick={() => setShowNewGoal(true)} className="flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-700 text-white text-[13px] font-medium rounded-md transition-colors shadow-sm"><Plus className="w-4 h-4"/> New goal</button>
        </div>
      </div>
